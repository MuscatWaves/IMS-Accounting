...
const PurchaseFromOtherBranchesReport = () => {
  ...
  useEffect(() => {
    document.title = "Accounting - Purchase From Other Branch Report";
    refetch(filter);
  ...
  const getData = async (values, page) => {
    ...
    try {
      const Data = await axios.get(
        `/api/clientpfob?search=${values.search}&page=${page}`,
        config
      );
      if (Data.status === 200) {
        setLoading(false);
  ...
  const columns = [
    ...
    },
    {
      title: "Recieved From",
      render: (record) => (
        <div className="text-grey">{record.receivedFrom}</div>
      ),
    },
    {
      title: "Location",
