...
const MerchantSummary = () => {
  ...
  useEffect(() => {
    document.title = "Accounting - Merchant Summary";
    refetch(filter);
  ...
  const getData = async (values, page) => {
    ...
    try {
      const Data = await axios.get(
        `/api/clientmsum?search=${values.search}&page=${page}`,
        config
      );
      if (Data.status === 200) {
        setLoading(false);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record) => <div className="text-grey">{record.grossSales}</div>,
    },
    {
