...
import SalesReportFormCreate from "./salesreportcreate";
  ...
  const getData = async (values, page) => {
    ...
    try {
      const Data = await axios.get(
        `/api/sr?search=${values.search}&page=${page}&clientId=${params.id}&entryDate=${values.entryDate}`,
        config
      );
      if (Data.status === 200) {
        setLoading(false);
