...
import ClosingStockFormCreate from "./closingstockcreate";
import ClosingStockFilter from "./closingStockFilter";
...
const ClosingStock = () => {
  ...
  useEffect(() => {
    document.title = "Accounting - Closing Stock";
    refetch(filter);
  ...
  const navigation = [
    ...
    {
      id: 4,
      name: "Closing Stock with values",
      active: true,
    },
  ];
  ...
  const getData = async (values, page) => {
    ...
    try {
      const Data = await axios.get(
        `/api/cswv?search=${values.search}&page=${page}&clientId=${params.id}&entryDate=${values.entryDate}`,
        config
      );
      if (Data.status === 200) {
        setLoading(false);
