...
import BalanceSheetFormCreate from "./balancesheetcreate";
...
const BalanceSheet = () => {
  ...
  useEffect(() => {
    document.title = "Accounting - Balance Sheet";
    refetch(filter);
  ...
  const getData = async (values, page) => {
    ...
    try {
      const Data = await axios.get(
        `/api/fsbsc?search=${values.search}&page=${page}&clientId=${params.id}&entryDate=${values.entryDate}`,
        config
      );
      if (Data.status === 200) {
        setLoading(false);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record) => (
        <div className="text-grey">{record.financialYear}</div>
      ),
    },
    {
      title: "Total Capital",
      render: (record) => (
        <div className="text-grey">{record.totalCapital}</div>
      ),
    },
    {
      title: "Total Fixed Assets",
      render: (record) => (
        <div className="text-grey">{record.totalFixedAssets}</div>
      ),
    },
    {
      title: "Total of Liabilities",
      render: (record) => (
        <div className="text-grey">{record.totalOfLiabilities}</div>
      ),
    },
    {
      title: "Total of Assets",
      render: (record) => (
        <div className="text-grey">{record.totalOfAssets}</div>
      ),
    },
    {
      title: "Client",
