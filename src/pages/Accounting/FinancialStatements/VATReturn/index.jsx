...
import VatReturnFormCreate from "./vatreturncreate";
import VatReturnFilter from "./vatReturnFilter";
...
const VATReturn = () => {
  ...
  useEffect(() => {
    document.title = "Accounting - VAT Return";
    refetch(filter);
  ...
  const getData = async (values, page) => {
    ...
    try {
      const Data = await axios.get(
        `/api/fsvrtc?search=${values.search}&page=${page}&clientId=${params.id}&entryDate=${values.entryDate}`,
        config
      );
      if (Data.status === 200) {
        setLoading(false);
  ...
  const columns = [
    ...
    {
      title: "Month",
      render: (record) => <div>{record.month}</div>,
    },
    {
      title: "Total Sales of Goods on 5%",
      render: (record) => (
        <div className="text-grey">{record.salesOfGoods}</div>
      ),
    },
    {
      title: "Total VAT on sales",
      render: (record) => <div className="text-grey">{record.vatOnSales}</div>,
    },
    {
      title: "Total Nil rated Sales",
      render: (record) => (
        <div className="text-grey">{record.nilRatedSales}</div>
      ),
    },
    {
      title: "Total Purchase of Goods on 5%",
      render: (record) => (
        <div className="text-grey">{record.purchaseOfGoods}</div>
      ),
    },
    {
      title: "Total VAT on Purchases",
      render: (record) => (
        <div className="text-grey">{record.vatOnPurchases}</div>
      ),
    },
    {
      title: "Total Nil rated purchase",
      render: (record) => (
        <div className="text-grey">{record.NilRatedPurchase}</div>
      ),
    },
    {
  ...
  const deleteData = async () => {
    ...
    await axios({
      method: "delete",
      url: `/api/fsvrtc/${deletionData.id}`,
      headers: {
