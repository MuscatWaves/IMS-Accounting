...
import IncomeStatementFormCreate from "./incomestatementcreate";
import IncomeStatementFilter from "./incomeStatementFilter";
...
const IncomeStatement = () => {
  ...
  useEffect(() => {
    document.title = "Accounting - Income Statement";
    refetch(filter);
  ...
  const getData = async (values, page) => {
    ...
    try {
      const Data = await axios.get(
        `/api/fsisc?search=${values.search}&page=${page}&clientId=${params.id}&entryDate=${values.entryDate}`,
        config
      );
      if (Data.status === 200) {
        setLoading(false);
  ...
  const columns = [
    ...
    {
      title: "Date/Month",
      render: (record) => <div>{record.dateMonth}</div>,
    },
    {
      title: "Total Sales",
      render: (record) => <div className="text-grey">{record.totalSales}</div>,
    },
    {
      title: "Total Purchase",
      render: (record) => (
        <div className="text-grey">{record.totalPurchase}</div>
      ),
    },
    {
      title: "Total Expense",
      render: (record) => (
        <div className="text-grey">{record.totalExpense}</div>
      ),
    },
    {
      title: "Net Profit",
    ...
    },
    {
      ...
      render: (record) => (
        <div className="text-grey">{record.netProfitPercentage}</div>
      ),
    },
    {
      title: "Total Amount of Income",
      render: (record) => (
        <div className="text-grey">{record.totalAmountOfIncome}</div>
      ),
    },
    {
      title: "Total Amount of Expense",
      render: (record) => (
        <div className="text-grey">{record.totalAmountOfExpense}</div>
      ),
    },
    {
      title: "Client",
